...
**ИИ для всех по ссылке:** вызовы модели идут **с сервера Vercel**, не из браузера пользователя. Локальный `.env` на вашем компьютере **не подхватывается** на продакшене. Чтобы кнопки ИИ работали у **любого**, кто открыл ваш сайт и вошёл в аккаунт, добавьте ключ LLM в **Settings → Environment Variables** (среда **Production**), сохраните и выполните **Redeploy** (см. таблицу в п. 3b).
